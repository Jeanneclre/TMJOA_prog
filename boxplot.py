...
input_file = './Predictions/40in_FScv_seeds/GlobEvaluation.csv'
...
for method in methods_list:
    ...
    # find every combination of method FS_PM starting with method in the column

    all_row = data[data['Model FS_PM'].str.endswith(method)]['Model FS_PM'].unique()
    F1_score =[]
    for name in all_row:

        F1_score.append(data[data['Model FS_PM']==name]['F1 test (O)'].values[0])
    # Box plot of the F1 score
    total_boxplot.append(F1_score)

fig = plt.figure(figsize =(10, 7))
#median line in dark, point in blue and larger median line
plt.boxplot(total_boxplot,patch_artist=True,boxprops=dict(facecolor='grey'),medianprops=dict(color='black',markersize=15),flierprops=dict(marker='o', markerfacecolor='blue'))
# limit y axis from 0.35 to0.65
plt.ylim(0.35,0.80)
# add name of the methods on the x axis
plt.xticks([i for i in range(1,len(method_name)+1)],method_name)
plt.show()
